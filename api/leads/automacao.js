...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const servico = sanitize(body.servico || 'automacao');
    ...
    const fileName = `lote-b2b-${servico}-${today}.csv`;
    const filePath = path.join(folder, fileName);
    const status = 'novo';
    const now = new Date().toLocaleString('sv');

    const header = 'Lote;Nome;Telefone;Cidade;Data;Status;Resposta;Valor_Estimado;Obs;Perfil;Plano;Email;Servico';
    const row = `${servico};${nome};${telefone};${cidade};${now};${status};;;;${perfil};${plano};${email};${servico}`;
