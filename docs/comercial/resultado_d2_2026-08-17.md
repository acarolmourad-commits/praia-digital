# Resultado D2 — Motor A
Data: 2026-08-17
Horário: 09:00

## Status do disparo

| lead_id | canal | enviado_em | resposta | tipo_resposta | servico_interesse | valor_potencial | estagio | proxima_acao | responsavel | objeção |
|---------|-------|------------|----------|---------------|-------------------|-----------------|---------|--------------|-------------|---------|
| 9 | OLX/WhatsApp | | | | | | | | | |
| 11 | OLX/WhatsApp | | | | | | | | | |
| 14 | Instagram/WhatsApp | | | | | | | | | |
| 15 | Facebook/WhatsApp | | | | | | | | | |
| 27 | Instagram/WhatsApp | | | | | | | | | |
| 29 | Facebook/WhatsApp | | | | | | | | | |

## Classificação pós-D2

### Respostas positivas → HANDOFF_HUMANO
- Leads: 
- Ação: comercial@praia.digital / (11) 95434-6288

### Pedidos de preço → RESPONDENDO_PRECO
- Leads:
- Ação: responder com preço aprovado

### Pedidos de agendamento → AGENDAMENTO_HUMANO
- Leads:
- Ação: confirmar disponibilidade

### Respostas negativas → ENCERRADO
- Leads:
- Motivo:

### Bloqueios → BLOQUEADO
- Leads:
- Motivo:

### Sem resposta → D5 em 20/08
- Leads:
- Ação: aguardar D5

## Métricas do dia
- Enviados D2: 
- Respostas: 
- Positivas: 
- Preços: 
- Agendamentos: 
- Negativas: 
- Bloqueios: 
- Handoffs: 

## Próximos passos
- [ ] Atualizar CRM com resultados
- [ ] Rodar `python scripts/follow_up_automacao.py`
- [ ] Rodar `python scripts/relatorio_diario_motor_b.py`
- [ ] Commit com mensagem padrão
- [ ] Preparar D5 para 20/08 se necessário
