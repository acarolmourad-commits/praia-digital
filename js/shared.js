    ...
    if (navMarker) {
      inject(navMarker, HEADER_URL, function(tmp) {
        ...
        if (header && navMarker.parentNode) {
          ...
          // Tó roda DEPOIS da injeção bem-sucedida, para nunca deixar a p�gina sem navegação.
...
(function () {
  ...
  function onlyRealListings() {
    try {
      ...
      var count = document.getElementById('resultsCount');
      if (count) {
        count.textContent = listings.length + (listings.length === 1 ? ' ioótvel verificado' : ' imóveis verificados');
...
// Praia Digital — Breadcrumbs centralizados v1.0
// Gera breadcrumb visível + JSON-LD BreadcrumbList em todas as páginas (exceto home).
(function(){
  function ready(fn){ if(document.readyState!=='loading'){fn();} else {document.addEventListener('DOMContentLoaded',fn);} }
  var LABELS = {
    'blog':'Blog','bairros':'Bairros','guias':'Guias','education':'Academy','formacoes':'Formações',
    'servicos':'Serviços','servicos-hermes':'Serviços Hermes','corretores':'Corretores','afiliados':'Afiliados',
    'produtos':'Produtos','ferramentas':'Ferramentas','ferramentas-gratuitas':'Ferramentas Gratuitas',
    'calculadoras':'Calculadoras','cases':'Cases','noticias':'Notícias','newsletter':'Newsletter',
    'imoveis':'Imóveis','investidores':'Investidores','proprietarios':'Proprietários','anfitrioes':'Anfitriões',
    'profissionais':'Profissionais','parcerias':'Parcerias','landings':'Landings','lp':'Landing Pages',
    'cidades':'Cidades','checklists':'Checklists','lead-magnets':'Materiais Gratuitos','compras':'Compras',
    'primeiro-imovel-litoral-sp-2026':'Primeiro Imóvel 2026','eventos-litoral-paulista-2026-2027':'Eventos 2026-2027',
    'contato':'Contato','faq':'FAQ','termos-uso':'Termos de Uso','politica-de-privacidade':'Privacidade'
  };
  function humanize(slug){
    slug = slug.replace(/\.html?$/i,'');
    if (LABELS[slug]) return LABELS[slug];
    return slug.replace(/-\d{4}(-\d{2})?(-\d{2})?$/,'').replace(/[-_]+/g,' ')
  ...
  ready(function(){
    try{
      ...
      var items=[{name:'Início',url:'https://praia.digital/'}];
      var acc='';
      segs.forEach(function(seg,i){
        acc+='/'+seg;
        var isLast = i===segs.length-1;
        var url='https://praia.digital'+acc;
        if(isLast && /\.html?$/i.test(seg)){} else {url+='/';}
        items.push({name:humanize(seg),url:url});
      });
      var last=items[items.length-1];
      var nav=document.createElement('nav');
      nav.className='pd-breadcrumb';
      nav.setAttribute('aria-label','Breadcrumb');
      nav.style.cssText='font-size:.85rem;color:#6b7280;padding:.6rem 1rem;max-width:1100px;margin:0 auto;display:flex;flex-wrap:wrap;gap:.35rem;align-items:center';
      items.forEach(function(it,i){
        if(i>0){var sep=document.createElement('span');sep.textContent='›';sep.style.color='#9ca3af';nav.appendChild(sep);}
        if(i<items.length-1){
          var a=document.createElement('a');a.href=it.url;a.textContent=it.name;
          a.style.cssText='color:#0077B6;text-decoration:none';nav.appendChild(a);
        } else {
          var span=document.createElement('span');span.textContent=it.name;span.setAttribute('aria-current','page');
          span.style.cssText='color:#374151;font-weight:600';nav.appendChild(span);
        }
      });
      var anchor=document.querySelector('header.pd-header')||document.querySelector('main')||document.body.firstChild;
      if(anchor && anchor.parentNode){ anchor.parentNode.insertBefore(nav, anchor.nextSibling); }
      else { document.body.insertBefore(nav, document.body.firstChild); }
      if(!document.querySelector('script[type="application/ld+json"][data-pd-breadcrumb]') &&
         !(document.querySelector('script[type="application/ld+json"]') && /BreadcrumbList/.test(document.head.innerHTML))){
        ...
        sc.setAttribute('data-pd-breadcrumb','1');sc.textContent=JSON.stringify(ld);
        document.head.appendChild(sc);
      }
    }catch(e){ if(window.__pdShared){window.__pdShared.push(['breadcrumb-error', String(e)]);} }
  });
})();
