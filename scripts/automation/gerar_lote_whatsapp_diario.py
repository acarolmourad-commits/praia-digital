#!/usr/bin/env python3
"""
Gera lote WhatsApp diário real com até 100 leads prontos para envio.
Entrada: docs/sales/leads-litoral-enriquecido.csv
Saída: docs/sales/lote-whatsapp-YYYY-MM-DD.html
"""
...
OUTPUT = BASE / f"docs/sales/lote-whatsapp-{TODAY}.html"
MAX_DAILY = 100
...
EXCLUDED_STATUSES = {"parceria_fechada", "nao_interessado", "cancelado", "convertido"}

rows = list(csv.DictReader(CSV_FILE.open(encoding="utf-8", errors="ignore")))
eligibles = [r for r in rows if r.get("whatsapp", "").strip() and r.get("status", "").lower() not in EXCLUDED_STATUSES]
eligibles.sort(key=lambda r: int(r.get("_score", 0) or 0), reverse=True)

# pick up to MAX_DAILY
alvos = eligibles[:MAX_DAILY]

# simple city distribution for quick insight
city_count = Counter(r.get("cidade", "") for r in alvos)
...
for r in alvos:
    ...
    cards.append(
        ...
        <div class="card-lead">
          ...
          <div class="small">Score: {html_mod.escape(score)} | Status: {html_mod.escape(status)}</div>
...
<div class="wrap">
  <div class="card">
    ...
    <p class="lead">Total no lote: {len(alvos)}. Distribuição por cidade: {summary}.</p>
    <p class="lead">Template base: "{TEMPLATE}"</p>
  </div>
  <div class="grid">
    {''.join(cards)}
  </div>
