#!/usr/bin/env python3
# Fase 2 — Auditoria SEO praia.digital (2026-09-24)
...
def keep(url):
    ...
    if url.endswith('/vendas.html'): return False            # canonical -> index.html
    if '/academy/' in url: return False                       # canonical -> /education/
    if CITY_DUP.search(url): return False                     # canonical -> /cidades/
    if LANDING_DUP.search(url): return False                  # consolidadas
    return True
