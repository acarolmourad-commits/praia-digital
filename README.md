...
- `fix-links-apply.yml` — correção automática de links sob demanda
- `seo-h1-apply.yml` — injeção de h1 acessível sob demanda
...
python audit_imoveis.py                # audita páginas de imóveis
python scripts/fix_broken_links.py     # corrige links internos (alta confiança; --dry-run para simular)
python scripts/seo_h1_autofix.py       # injeta h1 acessível onde falta (--dry-run)
```
